...
import { LeaderboardBoard } from './FifaLeaderboard';
...
export default function Dashboard({ onOpenAgent, onNavigate }: DashboardProps) {
  ...
  return (
    <div className="space-y-6 pb-10">
      ...
      {/* ── Creator Leaderboard (discovery happens on-platform) ──────────────── */}
      <div className="space-y-6">
        <div>
          <h2 className="text-3xl font-bold text-white tracking-tight">Creator Leaderboard</h2>
          <p className="text-zinc-400 text-sm mt-2 max-w-2xl">
            Fan-made cards ranked by the engagement they drive on TikTok — every one a Lenovo + FIFA billboard. Discovery happens on-platform; the cards travel, and remix pulls new fans back.
          </p>
        </div>
        ...
        {/* Create CTA → Studio */}
        <div className="relative rounded-[24px] overflow-hidden group cursor-pointer" onClick={() => onNavigate('studio')}>
          <div className="absolute inset-0" style={{ background: '#00C2A8' }} />
          ...
          <div className="relative z-10 flex items-center justify-between px-8 py-5">
            <div>
              <div style={{ fontSize: '9px', fontWeight: 800, letterSpacing: '0.28em', color: 'rgba(0,0,0,0.35)', textTransform: 'uppercase', marginBottom: '6px' }}>Studio · Describe your card</div>
              <div style={{ fontSize: '28px', fontWeight: 900, color: '#000', textTransform: 'uppercase', letterSpacing: '-0.02em', lineHeight: 1 }}>YOUR TURN.<br />MAKE ONE.</div>
            </div>
            <button
